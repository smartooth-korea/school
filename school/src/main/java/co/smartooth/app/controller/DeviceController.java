...
public class DeviceController {
	...
	/**
	 * 기능   : 측정자 사용하는 장비 정보 등록
	 * 작성자 : 정주현 
	 * 작성일 : 2022. 05. 19
	 * 수정일 : 2022. 08. 03
	 * 서버분리 : 2023. 08. 01
	 */
//	@PostMapping(value = {"/premium/device/insertDeviceInfo.do"})
	...
	public HashMap<String,Object> insertDeviceInfo(@RequestBody HashMap<String, Object> paramMap, HttpServletRequest request) throws Exception {
		...
		if(tmpTokenValidation) {
			try {
				...
				hm.put("code", "000");
				hm.put("msg", "기기 정보 등록 성공");
			} catch (Exception e) {
				hm.put("code", "500");
				hm.put("msg", "기기 정보를 등록하는데 실패했습니다.\n관리자에게 문의해주시기 바랍니다.");
				e.printStackTrace();
			}
		}else {
			hm.put("code", "400");
	...
	/**
	 * 기능   : 측정자 장비의 캘리브레이션 값 등록 또는 업데이트
	 * 작성자 : 정주현 
	 * 작성일 : 2022. 05. 19
	 * 수정일 : 2023. 08. 02
	 * 서버분리 : 2023. 08. 01
	...
	public HashMap<String,Object> insertCalibrationInfoValue(@RequestBody HashMap<String, Object> paramMap, HttpServletRequest request) throws Exception {
		...
		if (tmpTokenValidation) {
			...
			try {
				...
				hm.put("code", "000");
				hm.put("msg", "캘리브레이션 정보 등록 성공");
			} catch (Exception e) {
				hm.put("code", "500");
				hm.put("msg", "캘리브레이션 정보 등록에 실패했습니다.\n관리자에게 문의해주시기 바랍니다.");
				e.printStackTrace();
			}
		}else {
			hm.put("code", "400");
